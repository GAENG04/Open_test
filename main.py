...
class Calculator(QWidget):
    #Calculator 클래스를 QWidget을 상속받아 정의
    
    def __init__(self): #생성자 (객체 초기화)
        super().__init__()  #부모 클래스 QWidget의 생성자를 호출하여 초기화
    ...
    def initUI(self): #UI를 초기화하는 함수
        ...
        vbox = QVBoxLayout()
        vbox.addWidget(self.te1) #텍스트 입력 창을 수직 레이아웃에 추가
        vbox.addLayout(hbox) #수평 버튼 레이아웃을 수직 레이아웃에 추가
        vbox.addStretch(1) #레이아웃에 여백 추가

        self.setLayout(vbox) #수직 레이아웃을 윈도우의 레이아웃으로 설정
        self.setWindowTitle('2023108130') #윈도우의 제목 설정
        self.setWindowIcon(QIcon('icon.png')) #윈도우 아이콘 설정 (아이콘 파일이 필요)
...
if __name__=='__main__':
    ...
    view=Calculator() #Calculator 클래스 객체 생성
